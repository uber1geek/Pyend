...
(function(exports, $) {
  function Editor() {
    this.form = $('form#comment-form');
    this.editor = $('textarea#content');
    this.container = $('ul.comments');
    this.initialize();
  }
  ...
  Editor.prototype.setupMasonry = function() {
    ...
    imagesLoaded(this.container, function() {
      self.container.masonry({'itemSelector': '.comment'});
    });
  }

  Editor.prototype.setupComment = function() {
    var self = this;
    $('a.archive-comment').on('click', this.archiveComment);
  }

  Editor.prototype.archiveComment = function(e) {
    e.preventDefault();
    var elem = $(this);
  ...
  Editor.prototype.addComment = function() {
    var self = this;
    this.editor.css('color', '#464545');
    $.post(this.form.attr('target'), this.form.serialize(), function(data) {
      if (data.success) {
        self.editor.val('').focus();
        listElem = $(data.comment);
        listElem.find('a.archive-comment').on('click', self.archiveComment);
